...
- v0.6.3 IPC/Validation Hardening (2026-07-29): listener pipe 게시/종료 race를 공용 lock으로 제거하고 connect-only probe를 1초 bounded `ping` roundtrip으로 교체. `project validate`가 `data.valid=false`인데 성공을 반환하던 계약도 `TestFailed`/exit 1로 정정. Unity labs + 3-OS CI 검증, Windows #12/#13 reporter 재검증 대기. PR 대상 939 테스트.
