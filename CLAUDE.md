...
- Post-v0.6.2 IPC Hardening (listener shutdown race + bounded ping readiness probe): In Progress (Unity lab 검증, Windows #12/#13 재검증 대기)

최근 확정 사항 (최신 3개만 표시, 전체 이력은 `docs/internal/DEVELOPMENT.md` "슬라이스 이력" 참조):
- Post-v0.6.2 IPC Hardening (2026-07-29): listener pipe 게시/종료 race를 공용 lock으로 제거하고 connect-only probe를 1초 bounded `ping` roundtrip으로 교체. Unity lab domain reload/UI close 통과, 10회 status probe에서 expected pipe-close 경고 delta 0. Windows #12/#13 reporter 재검증 대기. PR 대상 938 테스트.
