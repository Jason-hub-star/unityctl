...
| 7 | it3 | **ConsoleAppFramework 5.3.3 신규 CLI verb dispatch 불가 (명령 수 상한 ~176 추정)**: `app.Add` 신규 verb가 `--help`엔 뜨는데 dispatch case 미생성("Unknown command"). **검증**: 무관한 이름 `zzdiag probe`를 **full solution 빌드**(`dotnet build unityctl.slnx`)해도 실패. 반면 count가 낮을 때 추가된 `spatial describe`·`play start/stop/pause`는 정상 → **ctl이 CAF 명령 수 천장(~176개)에 도달**한 것으로 강력 추정. 현재 176개 | **P1(전 개발 봉쇄 — 신규 CLI 명령 불가)** | **다음 최우선**: (a) count-limit 확정(기존 1개 제거 후 신규 dispatch되는지) (b) 우회 — 유사 명령 통합(action 파라미터로), 또는 CAF 5.x 최신/대체 프레임워크로 마이그레이션. 확정 전까지 신규 기능은 기존 명령 확장으로 |
